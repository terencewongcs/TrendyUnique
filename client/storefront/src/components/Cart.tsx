...
import { fetchCart, deleteOneFromCart } from '../app/slice/cart';
import { useAppDispatch, useAppSelector } from '../app/hooks';
import AddToCart from './AddToCart';
import { useGlobal } from '../hooks/useGlobal';

const taxRate = 0.1;
const couponMap: Map<string, number> = new Map([
  ['20OFF', 20],
  ['30OFF', 30],
  ['100OFF', 100],
]);
...
const Cart = forwardRef<CMethods>(function Cart(_props, ref) {
  ...
  const applyCoupon = () => {
    if (!couponCode) { setDiscount(0); setCouponError(''); return; }
    if (!couponMap.has(couponCode)) { setCouponError('Coupon does not exist!'); setDiscount(0); return; }
    setDiscount(couponMap.get(couponCode) || 0);
    setCouponError('');
  };
  ...
  return (
    <Transition show={isOpen} enter="duration-200 ease-out" enterFrom="opacity-0" enterTo="opacity-100"
      leave="duration-300 ease-out" leaveFrom="opacity-100" leaveTo="opacity-0">
      <Dialog open={isOpen} onClose={toggle} className="relative z-50">
        <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
        <div className="fixed inset-0 w-screen pt-1">
          <DialogPanel className="bg-white w-full absolute inset-x-0 top-24 bottom-32 md:top-0 md:right-0
            ...
            <section className="flex-1 overflow-y-auto scroll-smooth">
              <ul className="px-5 pt-10">
                {cartList.items.map((item) => (
                  <li className="flex mb-7" key={item.product._id}>
                    <img src={item.product.image} alt="" className="w-28 h-28 object-cover" />
                    <div className="ml-3 flex-1 flex flex-col justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <a className="text-base font-medium text-gray cursor-pointer underline"
                          onClick={() => { removeProduct(item.product._id); }}>Remove</a>
                      </div>
                    </div>
                  </li>
                ))}
              ...
              <section className="p-5">
                <ul className="text-base font-semibold text-black-common">
                  ...
                  <li className="flex mb-4 justify-between"><span>Estimated total</span>
                    <span>${(cartList.totalPrice * (1 + taxRate) - discount).toFixed(2)}</span></li>
                </ul>
                <button className="w-full bg-blue text-white text-sm font-bold rounded h-11"
                  onClick={goToCheckout}>Continue to checkout</button>
              </section>
            </section>
          </DialogPanel>
        </div>
