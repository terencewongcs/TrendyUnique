...
const ProductForm: FC<FormProps> = ({ id, submit }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setForm({ name: data.name, description: data.description, price: String(data.price), quantity: String(data.quantity), image: data.image });
      setCategory({ id: data.category, text: data.category });
      setPreviewImg(data.image);
    };
    showLoading(true);
    load().catch((e: unknown) => { showMessage(String(e)); }).finally(() => { showLoading(false); });
  }, [id]);

  const imgRegex = /^(http:\/\/|https:\/\/)/i;

  const uploadImg = () => {
    if (!imgRegex.test(form.image)) { setImageErr('Invalid Image Link'); setPreviewImg(''); }
    else { setImageErr(''); setPreviewImg(form.image); }
  };

  const checkForm = () => {
    let valid = true;
    if (!form.name) { setNameErr('Invalid Product Name'); valid = false; } else setNameErr('');
    if (!form.description) { setDescriptionErr('Invalid Product Description'); valid = false; } else setDescriptionErr('');
    if (!form.price || isNaN(Number(form.price))) { setPriceErr('Invalid Price'); valid = false; } else setPriceErr('');
    if (!form.quantity || isNaN(Number(form.quantity))) { setQuantityErr('Invalid Quantity'); valid = false; } else setQuantityErr('');
    if (!imgRegex.test(form.image)) { setImageErr('Invalid Image Link'); valid = false; } else setImageErr('');
    return valid;
  };
  ...
  const doSubmit = () => {
    ...
    const params: ProductCreateType = {
      name: form.name, description: form.description, category: category.id,
      price: Number(form.price), quantity: Number(form.quantity), image: form.image,
    };
    submit(params).then((ok) => {
      if (ok && !id) { setForm({ name: '', description: '', price: '', quantity: '', image: '' }); setPreviewImg(''); }
  ...
  return (
    <div className="bg-white w-full rounded-sm py-4 px-6">
      ...
      <div className="flex justify-center">
        {previewImg === '' ? (
          <div className="w-full md:w-3/5 h-52 my-4 border border-dashed border-gray rounded flex justify-center items-center flex-col">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-8 text-gray-border">
