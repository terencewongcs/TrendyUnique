  ...
  return (
    <div className="w-full">
      <Listbox value={selected} onChange={callback}>
        ...
        <Transition enter="transform transition duration-100 ease-out" enterFrom="opacity-0 scale-95"
          enterTo="opacity-100 scale-100" leave="transform transition duration-75 ease-in"
          leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
          <ListboxOptions anchor="bottom"
            className={clsx('w-[var(--button-width)] rounded-xl border bg-white p-1 [--anchor-gap:var(--spacing-1)] focus:outline-none border-gray-border')}>
            {options.map((item) => (
